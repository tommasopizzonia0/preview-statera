...
export const FeaturesGrid = () => {
  ...
  return (
    ...
    */
    <section className="w-full bg-emerald-100/25 border-y-[6px] border-emerald-500/50 shadow-[0_0_60px_rgba(16,185,129,0.3),inset_0_0_120px_rgba(16,185,129,0.2)] py-24 relative overflow-hidden animate-pulse-glow">
      
      {/* Background decorativo con luci soffuse */}
      <div className="absolute top-1/2 left-0 w-[500px] h-[500px] bg-emerald-400/20 rounded-full blur-[160px] -z-10" />
      <div className="absolute top-1/2 right-0 w-[500px] h-[500px] bg-emerald-400/20 rounded-full blur-[160px] -z-10" />

      <div className="max-w-[1200px] gap-10 grid grid-cols-12 grid-rows-2 px-8 mx-auto">
        ...
        {/* CARD 5: APP PAZIENTE (GRANDE) */}
        <Card isFooterBlurred className={`w-full h-[300px] col-span-12 sm:col-span-7 ${cardStyle}`}>
          <CardHeader className={headerGradient}>
            <p className="text-tiny text-emerald-300 uppercase font-black text-left w-full tracking-[0.2em]">Connettività</p>
            <h4 className="text-white font-black text-xl text-left w-full leading-tight">L&apos;App per i tuoi pazienti</h4>
          </CardHeader>
          <Image
            removeWrapper
            alt="Patient App"
          ...
          <CardFooter className="absolute bg-white bottom-0 z-10 border-t-2 border-emerald-500/40 backdrop-blur-md p-4">
            <div className="flex grow gap-3 items-center text-left">
              <div className="flex flex-col gap-0.5">
                <p className={`text-tiny ${textDark} font-black tracking-tight uppercase`}>Statera Patient</p>
                <p className="text-slate-600 text-tiny font-bold italic">Sincronizzazione in tempo reale.</p>
              </div>
            </div>
